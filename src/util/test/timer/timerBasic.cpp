...
void HL_Timer::init ()
{
	...
	//Set GPTimer Mode to a Normal timer - as opposed to a Cascaded
	//timer.
	GPTimer2->setMode(GPT_Normal);	

	//Set Reference Value of Timer Reference Register.
	GPTimer2->setReferenceValue(0xffff, TIMER_NOUNITS);
...
void* startTestTimer (void*) 
{
	...
	for (i = 0; i < MAX_THREAD1_TIMERS; i++)
	{
		...
		// Print the current value of TCN.
		printf("Before startTimer %d: Counter Value = %d\n",i, hl_timer->readCounter());
#else
		struct timeval tz;
	...
	while(count != (MAX_THREAD1_TIMERS))
	{
		if (queue1->empty() == false)
		{
			...
			printf("After timeOut %d: Counter Value = %d\n",count, hl_timer->readCounter());
#else
