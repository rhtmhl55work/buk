...
#define MAX_THREAD1_TIMERS 1000
#define MAX_THREAD2_TIMERS 7000 
#define MAX_THREAD3_TIMERS 6000
...
main (int argc, char **argv)
{
	...
	while (1)
	{
		...
		printf("\tTimeUnits\t Ab.Start Time \t Ab.TimeOut Time\t\n");		
		...
		for(int i = 0; i < MAX_TIMERS; i++)
		{
			...
			time_result[i].start_timer_diff = two - one;

			time_result[i].start_to_diff = three - one;
			
			printf("\t%ld \t %ld \t\t %ld \t\n",time_result[i].timeUnits, time_result[i].start_timer_diff, time_result[i].start_to_diff);
		}	
} // main
